...
export function normalizeStructuredLecture(
  ...
  // Cap phrases on lexical groups softly to 15 items for readability
  const rawPhrases = normalizePhrases(mainLexical.phrases, "lexical_groups");
  mainLexical.phrases = rawPhrases.slice(0, 15);
  ...
  // Soft cap the takeaways phrases to 15 items maximum
  const tkPhrases = normalizePhrases(mainTakeaways.phrases, "takeaways");
  mainTakeaways.phrases = tkPhrases.slice(0, 15);
