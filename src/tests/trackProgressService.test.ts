...
describe('Track Progress Metro Line Stepper Service', () => {
  ...
  it('Case 1: Only opened track - rawLyrics is empty', () => {
    ...
    expect(vm!.steps[2].status).toBe('upcoming');  // Words upcoming
    expect(vm!.steps[3].status).toBe('upcoming');  // Saved upcoming
  });
  ...
  it('Case 2: Lyrics fetched but analysis is not ready', () => {
    ...
    expect(vm!.steps[2].status).toBe('completed'); // Words is ready when lyrics are
    expect(vm!.steps[3].status).toBe('upcoming');  // Saved
  });
  ...
  it('Case 3: Analysis complete but no saved phrases', () => {
    ...
    expect(vm!.steps[2].status).toBe('completed'); // Words complete
